...
for ii = 1:length(markerfiles)
    ...
    if filter_bool
        ...
        % apply original preprocessing steps of Pascucci 2020 Comp. Biology
        % 1 linear detrend with cutoff 1 Hz
        detrendIn.detrendType = 'linear'; detrendIn.detrendCutoff = 1; detrendIn.detrendStepSize = 0.02;
        EEG_filt = removeTrend(EEG_downsample,detrendIn);
        % 2 line noise removal
        [dat] = ft_preproc_dftfilter(double(EEG_filt.data), conf.srate/conf.dsfactor, [50 100], 'dftreplace', 'neighbour_fft');
        EEG_filt.data = single(dat);
