...
function [com] = pop_viewprops_PR( EEG, typecomp, chanorcomp, spec_opt, erp_opt, scroll_event, classifier_name, fig)
...
if length(chanorcomp) > PLOTPERFIG
    for index = 1:PLOTPERFIG:length(chanorcomp)
        pop_viewprops_PR(EEG, typecomp, chanorcomp(index:min(length(chanorcomp),index+PLOTPERFIG-1)), ...
            spec_opt, erp_opt, scroll_event, classifier_name);
    end;
    com = sprintf('pop_viewprops_PR( %s, %d, %s, %s, %s, %d, ''%s'' )', ...
...
for ri = chanorcomp
	...
	if isempty( button )
        ...
        if typecomp
            topoplot( ri, EEG.chanlocs,'style','map','headrad',0.35,'electrodes','off','whitebk','on','shading','interp','emarker',{'.','k',6,1},'conv','on','chaninfo', EEG.chaninfo);
        else
            if plotelec
                topoplot( EEG.icawinv(:,ri), EEG.chanlocs, 'style','map','headrad',0.35,'electrodes','on','whitebk','on','shading','interp','emarker',{'.','k',6,1},'conv','on','chaninfo', EEG.chaninfo);
            else
                topoplot( EEG.icawinv(:,ri), EEG.chanlocs, 'style','map','headrad',0.35,'electrodes','on','whitebk','on','shading','interp','emarker',{'.','k',6,1},'conv','on','chaninfo', EEG.chaninfo);
            end;
            % labels
		...
		set( button, 'callback', {@pop_prop_extended_PR, EEG, typecomp, ri, NaN, spec_opt, erp_opt, scroll_event, classifier_name} );
	end;
    if typecomp
